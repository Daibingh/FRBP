...
%%%%%%%%%%%%%%%%%%%%%%%%%%LDA%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lm = nClass;
if MPCADADim > length(odrIdx) || MPCADADim <= 0
    MPCADADim = length(odrIdx);
    disp(['MPCALDA£º set MPCADADim to ', num2str(MPCADADim), ' £¡']);
end
if MPCADADim < nClass -1
    disp(['MPCALDA: MPCADADim = ', num2str(MPCADADim), ' < (nClass -1) = ', num2str(nClass -1), ' £¡']);
    lm = MPCADADim -1;
end
...
option=struct('disp',0);
% [LDAU, LDAV] = eigs(SW\SB, nClass-1, 'lm', option);
[LDAU, LDAV] = eig(SW\SB);
LDAU = LDAU(:, 1:lm-1);
LDAV = LDAV(1:lm-1, 1:lm-1);
